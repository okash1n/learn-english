...
TAURI_SIGNING_PRIVATE_KEY_PATH="$HOME/.tauri/solo-eikaiwa-updater.key"
...
[[ -f "$TAURI_SIGNING_PRIVATE_KEY_PATH" ]] || { echo "ERROR: TAURI_SIGNING_PRIVATE_KEY_PATH が存在しません: $TAURI_SIGNING_PRIVATE_KEY_PATH" >&2; exit 1; }
export APPLE_SIGNING_IDENTITY APPLE_API_KEY APPLE_API_ISSUER APPLE_API_KEY_PATH TAURI_SIGNING_PRIVATE_KEY_PATH
...
for p in "$APP" "$DMG" "$TARGZ" "$SIG"; do
  [[ -e "$p" ]] || { echo "ERROR: 生成物がありません: $p（.sig 欠落なら TAURI_SIGNING_PRIVATE_KEY_PATH を確認）" >&2; exit 1; }
done
echo "-- 署名・公証の検証"
