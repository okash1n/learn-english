...
3. ad-hoc 署名のため、初回起動は Gatekeeper に「開発元を確認できないため開けません」と止められます。その場合はダブルクリックではなく、Finder で `solo-eikaiwa.app` を**右クリック（または Control+クリック）→「開く」**を選んでください
...
データ（会話履歴・進捗・ダウンロード済みモデル・ログ）は `~/Library/Application Support/solo-eikaiwa` に保存されます。リポジトリを clone して動かす常駐運用・開発サーバとは完全に別の領域なので、混ざることはありません。
