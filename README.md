...
- **セッション継続**: プロバイダごとに継続の仕組みが異なる。**Claude**（既定）は Claude Agent SDK がセッションをディスクに永続化するため、サーバ再起動をまたいでも会話が復元される。**Codex** は `codex app-server`（常駐プロセス・実験的プロトコル）のネイティブ thread として保存され、`thread/resume` によりサーバ再起動後も同じ会話の続きとして復元される（Claude と同等のパリティ）。ただし app-server が使えない・応答しない場合は、その呼び出しだけ従来の `codex exec`（ワンショット）へ自動フォールバックする。フォールバック中はサーバのインメモリ・トランスクリプトを新スレッドの初回入力に畳み込む疑似継続になり、**サーバ再起動をまたぐと文脈は失われ新セッションとして継続される**（劣化動作）。**OpenAI 互換**はステートレスなため常にインメモリ・トランスクリプトで継続を再現し、サーバ再起動で会話履歴は失われる。
