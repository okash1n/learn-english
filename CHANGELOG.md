...
- **Codex のセッション永続化（サーバ再起動をまたいで会話が続く）**: 会話ロールに Codex を割り当てているとき、`codex app-server`（常駐プロセス・JSON-RPC の実験的プロトコル）のネイティブ thread として会話が保存されるようになった。サーバを再起動しても `thread/resume` で同じ会話の続きから再開できる（Claude Agent SDK と同等の継続性）。app-server が使えない・応答しない場合は、その呼び出しだけ従来の `codex exec`（ワンショット）へ自動でフォールバックする（フォールバック中はサーバのインメモリ・トランスクリプトで疑似継続するため、サーバ再起動をまたぐと文脈は失われ新セッション扱いになる）。動作確認済みの codex バージョンは `0.142.5`
